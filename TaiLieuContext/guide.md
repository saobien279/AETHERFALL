...
- `EntityManager.New(playerData: table, RaceRegistry: table)`
  ...
  - **Trả về (`return`):** Trả về một **Bảng `table` Entity hoàn chỉnh** gồm có `{Name, ID, Type, Race, MaxHP, CurrentHP, Energy, MaxEnergy, Inventory, Skillset, Stats, DamageReduction, ActiveCooldowns, StatusEffects, IsDead}`.
