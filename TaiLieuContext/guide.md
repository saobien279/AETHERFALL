# 🗺️ AETHERFALL - LẬP TRÌNH API & GUIDE

> **Cẩm nang Code (Dành cho AI & Người phát triển)**
> File này lưu trữ vị trí của toàn bộ các File cấu trúc, chức năng hàm, và đặc biệt là Sơ đồ móc nối tới hệ thống UI/Part lưu trên Roblox Studio mà Code không thể tự nhìn thấy.

---

## 1. CÂY THƯ MỤC LƯU TRỮ VẬT LÝ CHIẾU TRÊN ROBLOX STUDIO

Do AI không có khả năng quan sát không gian 3D và giao diện trực tiếp trên Roblox, bất kỳ dòng code UI/Part nào muốn trỏ tới đúng mục tiêu đều bắt buộc phải tham chiếu **Tuyệt đối** theo cây đồ thị (Hierarchy) dưới đây (Sử dụng `WaitForChild()` để truy cập):

### 📺 StarterGui (Cấu trúc Giao diện Tĩnh)
*Bản thiết kế (Blueprint) của UI, được vẽ trực tiếp trên Roblox Studio.*
- **ActionMenu**: Chứa con cái là Nút tớn công `AttackBtn`.
- **Heath**: Khung máu. Chứa `Frame` -> `FillBar`.
*(Lưu ý: Game này không gắn LocalScript vào trực tiếp trong các Frame này, mà điều khiển từ xa thông qua Controllers ở StarterPlayerScripts).*

### 📡 ReplicatedStorage (Giao thức mạng)
- `Events/ActionMenuEvent` **[RemoteEvent]**: UI ném tên kỹ năng (`skillID`) lên Server để xử lý.
- `Events/CombatEvents` **[RemoteEvent]**: Cáp quang truyền hiệu ứng từ Server về Client (UpdateHP, FadeToBlack).
- `Events/StartEncounterEvent` **[BindableEvent]**: Dây cáp ngầm nội bộ Server nối giữa Bãi Mìn và BattleServer.

### 🌍 Workspace (Hệ thống Vật thể / Sàn đấu 3D)
- **`WildSlime`** (Model): Chứa quái vật.
  - Bên trong chứa: `EncounterZone` (Gắn Tag làm mìn), `SlimePart`, và `HeathBar` (BillboardGui hiển thị máu lơ lửng).

### 🎮 StarterPlayer/StarterPlayerScripts (Bộ não Giao diện)
*Nơi chứa tất cả các hệ thống điều khiển tĩnh (Controllers) của người chơi. Tương tác với màn hình.*
- `ActionMenuController.client.luau`: Đăng ký sự kiện Click chuột, tìm nút `AttackBtn` và gọi `FireServer`.
- `HealthUIController.client.luau`: Bắt sự kiện mạng cập nhật máu bằng tỷ lệ %.
- `EnemyUIController.client.luau`: Cập nhật thanh máu BillboardGui treo trên đầu quái.
- `CinematicUIController.client.luau`: Quản lý Transition (Chuyển cảnh) đen màn hình khi đạp mìn.

### 💻 ServerScriptService (Trái tim Máy Chủ)
*Nơi chạy các tập lệnh chìm, kiểm soát An ninh mạng.*
- **`BattleServer.server.luau`**: Nghe lệnh từ `StartEncounterEvent`, tải Data và mở Lồng Bát Giác.
- **`EncounterManager.server.luau`**: Dùng `CollectionService` rà quét mìn `"EncounterZone"`, khóa chân, teleport và gọi Cinematic Fade.
- **`Modules/ProfileStore.luau`**: Bộ thư viện Version 2 (Session-Lock) chống Dupe.
- **`Modules/DataManager.luau`**: Thư viện lưu trữ, giao tiếp với `ProfileStore`.
- **`Modules/MonsterAI.luau`**: Não bộ quyết định đòn đánh của quái dựa trên Vòng quay Roulette (Weight).

---

## 2. TỪ ĐIỂN API & KIẾN TRÚC MODULES (`src/ReplicatedStorage/Modules/`)

Toàn bộ Backend lõi của game hiện đang được chia thành 4 Tầng cấu trúc độc lập. Dưới đây là các hàm bạn có thể gọi:

### 📁 TÀI LIỆU TẦNG CORE (Lõi vận hành trận đấu)
**1. Tệp `CombatManager.luau`**
*Loại File: Chứa các hàm xử lý Vòng lặp đấm đá, chịu đòn, sụt máu, và quản lý hiệu ứng trạng thái.*
- `CombatManager.UpdateHP(entity: table, amount: number)`
  - **Truyền vào:** `entity` (Một thực thể sống sinh ra từ EntityManager) và `amount` (Số máu cộng thêm hoặc nhận sát thương).
  - **Trả về (`return`):** Trả về `number` (Là CurrentHP còn lại).
  - **Nhiệm vụ:** Cộng/trừ máu an toàn với tính năng Kẹp trần `math.clamp` không cho máu quá MaxHP hoặc tụt âm. Check và vứt hàm `IsDead = true`.
  - **Hook:** `BeforeHeal` (chỉ khi `amount > 0`).
- `CombatManager.TakeDamage(defender: table, incomingDamage: number, damageType: string, attacker: table?)`
  - **Truyền vào:** Kẻ chịu đòn (`defender`), Sát thương đầu vào thô (`incomingDamage`), Hệ Sát Thương `damageType` (`"Physical"`, `"Magical"`, hoặc `"TrueDamage"`), và Kẻ tấn công (`attacker`, tùy chọn).
  - **Trả về (`return`):** `nil`.
  - **Nhiệm vụ:** Tìm áo Giáp của phe thủ để lấy `P_DEF`/`M_DEF` tương ứng với `damageType`. Gọi hàm `FinalDamage` và sau đó đem cục máu cuối cùng đẩy cho `UpdateHP` trừ.
  - **Hook:** `BeforeTakeDamage` (đầu hàm), `AfterTakeDamage` (cuối hàm).
- `CombatManager.DealDamage(attacker: table, defender: table, skillID: string)`
  - **Truyền vào:** Kẻ đánh (`attacker`), Kẻ thủ (`defender`) và Tên chiêu thức (`skillID`).
  - **Trả về (`return`):** `nil`.
  - **Nhiệm vụ:** Hàm khởi nguồn. Tra cứu Kỹ năng + Scale Vũ khí + Tính Dual-Scales / Diminishing Returns, sau đó đẩy đống Data Damage này vứt sang cho nhánh `TakeDamage`.
  - **Hook:** `BeforeDealDamage` (trước khi vung đòn), `AfterDealDamage` (sau khi đánh trúng).
- `CombatManager.RunModifierHook(entity: table, hookName: string, baseValue: number, context: table?)`
  - **Truyền vào:** Thực thể cần quét hiệu ứng, tên Hook (`"BeforeHeal"`, `"BeforeTakeDamage"`, `"BeforeDealDamage"`), giá trị gốc, và ngữ cảnh phụ.
  - **Trả về (`return`):** `number` (Giá trị đã bị sửa đổi bởi các hiệu ứng).
  - **Nhiệm vụ:** Cỗ máy "Sửa Số". Quét qua tất cả hiệu ứng đang có trên entity, gọi các Modifier Hook tương ứng để bóp méo con số.
- `CombatManager.RunActionHook(entity: table, hookName: string, context: table?)`
  - **Truyền vào:** Thực thể cần quét, tên Hook (`"OnTurnStart"`, `"AfterTakeDamage"`, `"AfterDealDamage"`), và ngữ cảnh phụ.
  - **Trả về (`return`):** `nil` (Tự thi hành các lệnh Action bên trong).
  - **Nhiệm vụ:** Cỗ máy "Thi Hành Án". Gom nhặt các Bảng Lệnh (`{Action="TakeTrueDamage"}`, `{Action="ReflectDamage"}`) từ hiệu ứng và thực thi chúng.
- `CombatManager.ApplyStatusEffect(entity: table, effectName: string, stacks: number?, duration: number?)`
  - **Truyền vào:** Thực thể, tên hiệu ứng, số Stack (mặc định 1), Duration (chỉ dùng cho Shield).
  - **Trả về (`return`):** `nil`.
  - **Nhiệm vụ:** Gán hiệu ứng lên entity. Nếu đã dính rồi thì cộng dồn Stack (trừ Shield). Gọi `OnApply` nếu lần đầu.
- `CombatManager.RemoveStatusEffect(entity: table, effectName: string)`
  - **Truyền vào:** Thực thể và tên hiệu ứng cần gỡ.
  - **Trả về (`return`):** `nil`.
  - **Nhiệm vụ:** Gỡ hiệu ứng khỏi entity. Gọi `OnRemove` để dọn dẹp (cộng trả lại giáp cho Sunder).

**2. Tệp `EntityManager.luau`**
*Loại File: Chứa các hàm liên quan đến nhào nặn thân thể thực thể trận đánh.*
- `EntityManager.New(playerData: table, RaceRegistry: table)`
  - **Truyền vào:** Dữ liệu cấp bậc/tộc hiện tại của người chơi (`playerData`) từ DataTemplate và biến môi trường `RaceRegistry`.
  - **Trả về (`return`):** Trả về một **Bảng `table` Entity hoàn chỉnh** gồm có `{Name, ID, Type, Race, MaxHP, CurrentHP, Energy, MaxEnergy, Inventory, Skillset, Stats, DamageReduction, ActiveCooldowns, StatusEffects, IsDead, P_DEF, M_DEF, WeaponBaseDamage, WeaponBuffMultiplier}`.
  - **Lưu ý:** `.ID` (Network ID) luôn độc lập với `.Name` (Tên hiển thị RP). Hệ thống UI dùng `.ID` để đối chiếu, không bao giờ dùng `.Name`.
  - **Nhiệm vụ:** Sinh ra một con cờ (Thực thể) hoàn thiện có gắn máu dâng theo END và chứa toàn bộ Trạng thái trước khi thảy nó lên sàn đấu. Chỉ số trang bị (P_DEF, M_DEF, Weapon) được cache sẵn vào entity lúc tạo thông qua `StatsCalculator.GetEquipmentModifiers`.

**3. Tệp `DataManager.luau` (Kho Dữ Liệu Bộ Nhớ)**
*Loại File: Module kết nối và quản lý ProfileStore.*
- `DataManager.Init()`
  - **Nhiệm vụ:** Bắt buộc phải gọi 1 lần duy nhất ở `BattleServer` hoặc `MainServer`. Nó sẽ mở khoen cài sự kiện PlayerAdded/Removing để bắt đầu load Data.
- `DataManager.GetProfile(player: Player)`
  - **Trả về (`return`):** Trả ra một `Profile` (bên trong có `.Data` lưu toàn bộ thông tin gốc). Trả `nil` nếu chưa load xong.
  - **Nhiệm vụ:** Lấy dữ liệu của người chơi. Hiện tại `BattleServer` gọi trực tiếp 1 lần (vì `EncounterManager` chỉ kích hoạt sau khi người chơi đã di chuyển, tức Data chắc chắn đã load xong).

**4. Tệp `TurnbaseEngine.luau` & Hệ thống `BattleServer`**
*Loại File: Quản lý Vòng lặp State Machine.*
- `TurnbaseEngine.SetState(battleState, newState)`
  - **Nhiệm vụ:** Đổi State. Bất cứ khi nào chuyển State sang Player_Turn hoặc Enemy_Turn, nó gọi hàm cộng MP tự động.
- `TurnbaseEngine.ProcessPlayerTurn(battleState, skillID)`
  - **Nhiệm vụ:** Hàm kết nối trực tiếp với Điện thoại `ActionMenuEvent`. Nó xét duyệt qua `SkillProcessor`, nếu cạn Mana đập về chữ `false`, còn nếu đủ Mana thì trừ phí và gọi Đánh.

### 📁 TÀI LIỆU TẦNG UTILS (Hỗ trợ Tính toán)
**Tệp `StatsCalculator.luau`**
- `StatsCalculator.GetTotalStats(playerData: table, RaceRegistry: table)`
  - **Truyền vào:** Hồ sơ nhân vật `playerData`.
  - **Trả về (`return`):** `table` (Trả ra 5 dòng Point nguyên bản: `STR`, `ARC`, `END`, `SPD`, `LCK` sau khi cộng thưởng bị động từ Tộc).
- `StatsCalculator.CalculateOutputDamage(skillBase: number, statBonus: number, weaponBase: number, weaponBuff: number)`
  - **Truyền vào:** Sát thương gốc Skill, Hệ số Buff chỉ số %, Sát thương gốc Vũ khí, Phép chấn Vũ khí %.
  - **Trả về (`return`):** `number` (Sát thương Thô).
- `StatsCalculator.calculateFinalDamage(incomingDamage: number, defenderDefense: number, defenderDR: number)`
  - **Truyền vào:** Máu gốc trước đánh, Chỉ số Thủ của Giáp, % Giảm sát thương.
  - **Trả về (`return`):** `number` (Sát thương Máu thật qua kẹp làm tròn).
- `StatsCalculator.GetEquipmentModifiers(playerEquipment: table, EquipmentRegistry: table)`
  - **Truyền vào:** Dữ liệu trang bị của người chơi và từ điển EquipmentRegistry.
  - **Trả về (`return`):** `table` (Chứa `P_DEF`, `M_DEF`, `SpeedBonus`, `WeaponBaseDamage`, `WeaponBuffMultiplier`).
  - **Nhiệm vụ:** Trích xuất chỉ số trang bị từ Registry, trả về bảng chỉ số gọn để cache vào Entity.

**Tệp `SkillProcessor.luau` (Chú Cảnh sát Gác cổng)**
- `SkillProcessor.CanUseSkill(attacker: table, skillName: string, registry: table)`
  - **Trả về (`return`):** `boolean` (Được xài hay không), `string` (Lý do báo lỗi: Hết mana, dính Silence...).
- `SkillProcessor.ConsumeCost(attacker: table, skillName: string, registry: table)`
  - **Nhiệm vụ:** Trực tiếp trừ Energy và nhét tên chiêu vào nợ `ActiveCooldowns`.
- `SkillProcessor.ProcessStartTurn(entity: table)`
  - **Nhiệm vụ:** Gọi vào mỗi đầu hiệp. Tự động hồi 1 Energy (Tối đa 6 Energy) và xóa nợ Cooldown cũ đi 1 hiệp.

### 📁 TÀI LIỆU TẦNG DATA & REGISTRIES (Chỉ lưu trữ tĩnh)
- `Data/DataTemplate.luau`: Tệp nháp Cấu trúc Database (Inventory/Stats) rỗng lúc NewGame. Được `DataManager` và `ProfileStore` chắp vá Reconcile gọi làm mẫu.
- `SkillRegistry.luau`: Chứa định nghĩa `BasePower`, `DamageType`, Cost.
- `RaceRegistry.luau`: Chứa các hệ số Buff chủng tộc `Passives`.
- `EquipmentRegistry.luau`: Chứa định nghĩa của `Weapons`, `Armors`, và 4 Slot `Gears`.
- `StatusEffectRegistry.luau`: Chứa định nghĩa tất cả hiệu ứng trạng thái (DoT, Debuff, CC, Buff) với kiến trúc Hook. Xem chi tiết tại `aboutproject.md` mục 14.
