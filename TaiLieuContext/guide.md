...
- `DataManager.GetProfile(player: Player)`
  - **Trả về (`return`):** Trả ra một `Profile` (bên trong có `.Data` lưu toàn bộ thông tin gốc).
  - **Nhiệm vụ:** Lấy dữ liệu của người chơi bất cứ lúc nào. Nếu người chơi chưa load xong, nó sẽ trả ra `nil`. Bắt buộc phải dùng cơ chế vòng lặp `repeat task.wait() until` để rình rập ở các ServerScripts.
